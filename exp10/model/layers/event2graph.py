...
from libs.graph.ev_graph import SlidingWindowGraph
...
def denormalize_pos(events):
    ...
    denorm = torch.tensor([int(events.width[0]), int(events.height[0]), int(events.time_window[0])],
                          device=events.pos.device)
    return (denorm.view(1, -1) * events.pos + 1e-3).int()


class Event2Graph(torch.nn.Module):
    ...
    def forward(self, events: Data, reset=True):
        ...
        # src_i <= dst_i
        # dst_i <= dst_j if i<j
        events.edge_index = self.graph_creators.forward(events.batch.int(), pos, delete_nodes=False,
                                                        collect_edges=reset)
